(() => {
    ...
    static resolve(base2, compartments, oldState) {
      ...
      if (oldState) {
        let canReuse = (id2, depth) => {
          if (depth > 7)
            return false;
          let addr2 = address[id2];
          if (!(addr2 & 1))
            return dynamicDeps[addr2 >> 1].every((id3) => canReuse(id3, depth + 1));
          let oldAddr = oldState.config.address[id2];
          return oldAddr != null && getAddr(oldState, oldAddr) == staticValues[addr2 >> 1];
        };
  ...
  var WidgetBufferView = class extends InlineView {
    constructor(side) {
      super();
      this.side = side;
    }
  ...
  var Decoration = class extends RangeValue {
    ...
    static replace(spec) {
      ...
      let startSide = 1e8 * (start ? -1 : 1) * (block ? 2 : 1);
      let endSide = 1e8 * (end ? 1 : -1) * (block ? 2 : 1);
  ...
  function moveVertically(view, start, forward, distance) {
    ...
    if (startCoords) {
      ...
      let line2 = view.viewState.lineBlockAt(startPos - docTop);
      if (goal == null)
  ...
  var ViewState = class {
    ...
    update(update, scrollTarget = null) {
      ...
      if (!update.changes.empty || update.flags & 2 || viewport.from != this.viewport.from || viewport.to != this.viewport.to)
        this.updateViewportLines();
    ...
    ensureLineGaps(current) {
      ...
      for (let line2 of this.viewportLines) {
        ...
        if (this.heightOracle.lineWrapping) {
          ...
          viewFrom = findPosition(structure, (this.visibleTop - line2.top - marginHeight) / line2.height);
          viewTo = findPosition(structure, (this.visibleBottom - line2.top + marginHeight) / line2.height);
        } else {
        ...
        if (viewFrom > line2.from)
          outside.push({ from: line2.from, to: viewFrom });
        if (viewTo < line2.to)
          outside.push({ from: viewTo, to: line2.to });
        let sel = this.state.selection.main;
        if (sel.from >= line2.from && sel.from <= line2.to)
          cutRange(outside, sel.from - 10, sel.from + 10);
        if (!sel.empty && sel.to >= line2.from && sel.to <= line2.to)
          cutRange(outside, sel.to - 10, sel.to + 10);
        for (let { from, to } of outside)
          if (to - from > 1e3) {
            gaps.push(find(current, (gap) => gap.from >= line2.from && gap.to <= line2.to && Math.abs(gap.from - from) < 1e3 && Math.abs(gap.to - to) < 1e3) || new LineGap(from, to, this.gapSize(line2, from, to, structure)));
          }
      }
      return gaps;
    }
  ...
  var EditorView = class {
    ...
    viewportLines(f, docTop) {
      let top2 = ensureTop(docTop, this);
      for (let line2 of this.viewportLineBlocks)
        f(line2.moveY(top2));
    }
      ...
      for (let parent = this; ; ) {
        for (let { children, positions } = parent.node, e = dir > 0 ? children.length : -1; i != e; i += dir) {
          ...
          if (next3 instanceof TreeBuffer) {
            ...
            let mounted;
            if (!(mode & 1) && next3.props && (mounted = next3.prop(NodeProp.mounted)) && !mounted.overlay)
  ...
  var parseWorker = /* @__PURE__ */ ViewPlugin.fromClass(class ParseWorker {
    ...
    work(deadline) {
      ...
      if (field.tree == field.context.tree && field.context.treeLen >= vpTo + 1e5)
        return;
    ...
    highlightRange(cursor, from, to, inheritedClass, depth, scope) {
      ...
      if (mounted && mounted.overlay) {
        ...
        for (let i = 0, pos = start; ; i++) {
          ...
          pos = next3.to + start;
          if (pos > from) {
            this.highlightRange(inner.cursor, Math.max(from, next3.from + start), Math.min(to, pos), inheritedClass, depth, mounted.tree.type);
...
`, error);
  ...
  var tooltipPlugin = /* @__PURE__ */ ViewPlugin.fromClass(class {
    ...
    writeMeasure(measured) {
      ...
      for (let i = 0; i < this.manager.tooltips.length; i++) {
        ...
        if (!pos || pos.bottom <= Math.max(editor2.top, space.top) || pos.top >= Math.min(editor2.bottom, space.bottom) || pos.right <= Math.max(editor2.left, space.left) || pos.left >= Math.min(editor2.right, space.right)) {
  ...
  var parser = LRParser.deserialize({
    version: 13,
    states: "8SOVQROOOzQRO'#ClO#aQRO'#CmO#|QRO'#CmO%_QRO'#CmO%iQRO'#CmO%sQRO'#CuOOQP'#Cx'#CxO'fQRO'#DWOOQO'#DW'#DWQ'vQQOOOOQP,59W,59WO%sQRO,59aO(OQRO,59cOOQP'#Cy'#CyO(aQRO'#CyO(xQRO'#CpO(OQRO'#CnO)ZQTO'#CpO+cQRO'#DYO*eQRO'#DYOOQP,59X,59XO!VQRO,59XO+jQRO'#CpO+{QTO'#CpO-SQRO'#CtO-hQQO'#CtO-mQRO'#DcO(OQRO'#DcO.kQRO'#DcO.uQRO'#CoO0OQTO'#CoO0]QQO'#CqO0sQRO,59XO0zQRO'#CoO1]QTO'#CoO1mQRO'#DhOOQP,59a,59aOOQP-E6v-E6vOOQO,59r,59rOVQRO'#DPQ'vQQOOOOQP1G.{1G.{OOQP1G.}1G.}OOQP,59],59]O2eQQO,59]OOQP-E6w-E6wO2mQTO,59[OOQP,59Y,59YOOQP'#Cz'#CzO2mQTO,59[O3wQRO,59[O4YQRO,59]O4bQRO'#CrO4gQRO'#C|O5gQRO,59tO6_QRO,59tO6iQRO,59tOOQP1G.s1G.sO6pQTO,59[O6pQTO,59[O7wQRO,59[O8YQRO'#DeO8nQSO'#DeO8yQQO,59`O-SQRO,59`O9OQRO'#C}O:RQRO,59}O:yQRO,59}O;TQRO,59}O;[QRO,59}O(OQRO,59}O;fQTO,59ZO;fQTO,59ZO<gQRO,59ZO<xQRO1G.sO=pQTO,59ZO=pQTO,59ZO(OQRO,59ZO>tQRO'#DOO?tQRO,5:SOOQO,59k,59kOOQO-E6}-E6}O@lQRO'#C{O@zQQO1G.wOOQP1G.w1G.wOASQTO1G.vO3wQRO1G.vOOQP-E6x-E6xOOQP1G.v1G.vO@zQQO1G.wOB^QQO,59^OCaQRO,59hOBcQRO,59hOOQP-E6z-E6zOChQRO1G/`OChQRO1G/`OD`QRO1G/`ODgQTO1G.vO7wQRO1G.vOOQP1G.y1G.yOEnQSO,5:POEnQSO,5:PO-SQRO,5:POOQP1G.z1G.zOEyQQO1G.zOFOQRO,59iO(OQRO,59iOF|QRO,59iOOQP-E6{-E6{OGWQRO1G/iOGWQRO1G/iOHOQRO1G/iOHVQRO1G/iOHaQRO1G/iOIcQTO1G.uO<gQRO1G.uOOQP1G.u1G.uOIpQTO1G.uO(OQRO1G.uOOQP,59j,59jOOQP-E6|-E6|OOQO,59g,59gOOQO-E6y-E6yOOQP7+$c7+$cO3wQRO7+$bOOQP7+$b7+$bOJQQQO7+$cOOQP1G.x1G.xOKTQRO1G/SOJYQRO1G/SOK[QRO7+$zOK[QRO7+$zO7wQRO7+$bOOQP7+$e7+$eOLSQSO1G/kO-SQRO1G/kOOQO1G/k1G/kOOQP7+$f7+$fOMYQRO1G/TOL_QRO1G/TOMaQRO1G/TO(OQRO1G/TON_QRO7+%TON_QRO7+%TO! VQRO7+%TO! ^