(() => {
  ...
  var readString = (string2) => {
    ...
    let output = [];
    let matches = string2.slice(1, -1).match(/(\\(?:x[0-9a-f]{1,2}|[0-7]{1,3}|u[0-9a-f]{1,8}|.?))|[^\\]+/ig);
    if (matches)
      matches.forEach((x) => {
        if (x[0] == "\\") {
          x = x.slice(1);
  ...
  function Expression(instr, minFloatPrec = 0, expectMemory = false) {
    ...
    while (token !== "," && token !== "\n" && token !== ";") {
      if (!lastWasNum && unaries.hasOwnProperty(token)) {
        ...
        if (!lastWasNum) {
          if (!instr.syntax.intel && expectMemory && opStack.length > 0 && opStack[opStack.length - 1].bracket)
            break;
