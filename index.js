(() => {
  ...
  var Statement = class {
    constructor(addr = 0, maxSize = 0, range = new Range3(), error = null) {
  ...
  var Symbol2 = class extends Statement {
    record;
    constructor(addr, name2, range, opcodeRange, isLabel = false) {
      super(addr, 0, range);
      this.name = name2;
      let uses = [];
      try {
  ...
  Operation.prototype.fit = function(operands, instr, vexInfo) {
    ...
    let reg = null, rm2 = null, vex = this.vexBase, imms = [], correctedOpcode = this.code, evexImm = null;
    let extendOp = false;
    let operand;
    for (i = 0; i < operands.length; i++) {
  ...
  Operation.prototype.generateRelative = function(operand, instr) {
    let target = operand.value.value - BigInt(instr.address + ((this.code > 255 ? 2 : 1) + (this.prefix !== null ? 1 : 0)));
...
g nle`.split("\n");
  ...
  var Instruction = class extends Statement {
    ...
    interpret() {
      ...
      let forceImmToRel = this.syntax.intel && mnemonics2.some((mnemonic) => mnemonic.relative);
            ...
            if (isDir) {
              if (currSyntax.intel ? lowerTok == "%assign" : lowerTok == ".equ" || lowerTok == ".set") {
                ...
                if (!currSyntax.intel && next() !== ",")
                  throw new ASMError("Expected ','");
                addInstruction(new Symbol2(currAddr, opcode, pos, opcodePos));
              } else
              ...
              next();
              if (token == ":")
                addInstruction(new Symbol2(currAddr, opcode, pos, pos, true), false);
              else if (token == "=" || currSyntax.intel && token.toLowerCase() == "equ")
                addInstruction(new Symbol2(currAddr, opcode, pos, pos));
...
`, e);
  ...
  var parser = LRParser.deserialize({
    version: 13,
    states: "9bOVQROOOzQRO'#CsOOQP'#Cv'#CvO!VQRO'#CjO#iQRO'#CjO#pQRO'#CjO$|QRO'#CjO%WQRO'#CjO%bQRO'#CrO&SQRO'#CrOOQP'#DV'#DVO&XQRO'#DVQ&gQROOOOQP,59T,59TO&oQRO,59aOOQP-E6t-E6tO'QQRO,59UO#pQRO,59UO'jQRO,59UO'tQRO,59UOOQP'#Cw'#CwO(OQRO'#CwO(gQRO'#CmO&oQRO'#CkO(xQTO'#CmO*UQRO'#DXO)mQRO'#DXOOQP,59U,59UO!tQRO,59UO*]QRO'#CmO*nQTO'#CmO+`QRO'#CqO+tQQO'#CqO+yQRO'#DcO&oQRO'#DcO,vQRO'#DcO-QQRO'#ClO-tQTO'#ClO.RQQO'#CnO.iQRO,59UO.pQRO'#ClO/RQTO'#ClO/cQRO'#DhOOQP,59^,59^O%bQRO,59^OOQP,59q,59qOVQRO'#C}Q&gQROOOOQP1G.{1G.{OOQP1G.p1G.pO!tQRO1G.pO/tQRO1G.pOOQP,59Y,59YO/{QQO,59YOOQP-E6u-E6uO0TQTO,59XOOQP,59V,59VOOQP'#Cx'#CxO0TQTO,59XO0xQRO,59XO1ZQRO,59YO1cQRO'#CoO1hQRO'#CzO2RQRO,59sO2dQRO,59sO2nQRO,59sO2uQTO,59XO2uQTO,59XO3gQRO,59XO3xQRO'#DeO4^QSO'#DeO4iQQO,59]O+`QRO,59]O4nQRO'#C{O5[QRO,59}O5mQRO,59}O5wQRO,59}O6OQRO,59}O&oQRO,59}O6YQTO,59WO6YQTO,59WO6tQRO,59WO7VQRO1G.pO7hQTO,59WO7hQTO,59WO&oQRO,59WO8VQRO'#C|O8mQRO,5